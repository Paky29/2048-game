...
class Agent_tony:
    def __init__(self):
        ...
        self.target_model = DeepQLearningNetwork(lr=self.lr, action_space=4, momentum=self.momentum)
    ...
    def learn(self):
        ...
        targets = self.target_model(states).detach()
        # Calcoliamo i valori Q massimi per i nuovi stati
        new_state_values = self.target_model(new_states).detach().max(1)[0]
    ...
    def target_train(self):
        # Ottieni i parametri (pesi) dei modelli
        main_model_params = self.main_model.state_dict()
        target_model_params = self.target_model.state_dict()

        # Aggiornamento ponderato dei pesi del modello target
        for name in main_model_params:
            target_model_params[name] = self.tau * main_model_params[name] + (1 - self.tau) * target_model_params[name]

        # Imposta i nuovi pesi aggiornati nel modello target
